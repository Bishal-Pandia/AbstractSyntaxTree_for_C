...
(*A wrapper function to convert infix to perfix and then parse*)          
let wrapper l = 
...
(* printf("Lahari"); *)
let test_4 () = 
  let e = [Print_generic(Print_Lexer); Token_generic([Id_token("Lahari")])] in
  parser_main e;;
...
let test_5 () =
let e = [
  While_generic(While_Lexer); Condition_generic(Id("a"), Greaterthan, Num(3)); Token_generic([Id_token("a"); Operator(Add); Num_token(1)])
] in
parser_main e;; 
...
let test_5 () =
  let e = [
    While_generic(While_Lexer); Condition_generic(Id("a"), Greaterthan, Num(3)); Expression_generic(BinaryOp(Id("y"), Assign, BinaryOp(Id("a"), Add,Num(2))))
  ] in
  parser_main e;; 
...
let test_6 () = 
  let e = [
    If_generic(If_Lexer); Condition_generic(Id("x"), Equal, Num(5)); Token_generic([Id_token("x"); Operator(Add); Num_token(1)]); Token_generic([Id_token("x"); Operator(Assign); Num_token(5)])
  ] in 
  parser_main e;;
...
let test_8 () = 
  let e = [ 
    Print_generic(Print_Lexer); Expression_generic(BinaryOp(Id("y"), Assign, BinaryOp(Id("x"), Add,Num(2))));
  ] in
  parser_main e;;

(*{10/2 -3};*)
let test_9 () =
  let e = [Others(Lparen); Token_generic([Num_token(10); Operator(Divide); Num_token(2); Operator(Subtract); Num_token(3)]); Others(Rparen)] in
    parser_main e;;

(*y=x+2;*)
let test_10 () = 
  let e = [
    Expression_generic(BinaryOp(Id("y"), Assign, BinaryOp(Id("x"), Add,Num(2))))
  ] in
  parser_main e;;
